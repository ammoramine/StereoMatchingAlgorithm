...
they contain only one half of the DCT coefficients (because of hermitian symmetry)
*/
...
{
    // If zoom < 1, we need to filter the signal
    float zoom = (float) w_out/w_in;
    int filter_needed = (zoom < 1);
    float s = 0; // Gaussian std deviation
    if (filter_needed)
		s = 0.8*sqrt((1/zoom)*(1/zoom)-1);
    ...
    for (int row = 0; row < h; row++)
    {
        ...
        {
            line_in[i] = in[row*w_in+i];
            line_in[n_in-1-i] = in[row*w_in+i];
        }
