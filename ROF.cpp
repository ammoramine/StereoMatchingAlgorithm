...
ROF::ROF(const double &tau,const double &l,int lengthROF)
// a should be a vector of positive
{
	 m_a=std::vector<double>(lengthROF,tau);
	 m_b=std::vector<double>(lengthROF,tau*l);
...
void ROF::initForwardPass()
{
	...
	m_lambdaMinVect.push_back((m_b[0]+m_omegaijMinus[0])/(m_a[0]+m_message[0]));
	m_lambdaPlusVect.push_back((m_b[0]+m_omegaijPlus[0])/(m_a[0]+m_message[0]));
...

{
	...
	double bi=m_a[m_index_current_node];
	...
	while(mLambdaCurrent<wikMin and p<m_message.size())//the lowest possible index for lambda is 1 and the bigger one is  m.size()-2, if it reachs m_message.size(), it means all the elements are below wikMin
	{
...

{
	...
	while(mLambdaCurrent>wikPlus and r>=0)//the lowest possible index for lambda is 1 and the bigger one is  m_message.size()-2, if it reachs -1, it means all the elements are above wikPlus
	{
...
double ROF::computeLambdaPlus(const double &lastComputedMLambda)
{
	...
	if (m_biggestIndexCurrentNode==-1)// all the element are bigger then wijPlus
	{
		...
		double slopeBeforeLambdaMax=m_message[m_smallestIndexCurrentNode+1]+m_aBar_current_node;

		lambdaPlusCurrentNode=lambdaMin+(wijPlus-mLambdaMin)/(slopeBeforeLambdaMax);// lambdaMax is after the biggest breakpoint
	}	
	else // m_smallestIndexCurrentNode is between 1 and m.size()-2
...
{
	if ( (m_lambdaMinVect[m_lambdaMinVect.size()-1]-m_lambdaPlusVect[m_lambdaPlusVect.size()-1])>0.0001 )
			{
